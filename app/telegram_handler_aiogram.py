...
# --- Общие команды (обрабатываются в любом состоянии) ---
@common_router.message(CommandStart())
async def handle_start(message: Message, state: FSMContext):
    chat_id = message.chat.id
    logging.info(f"/start from {chat_id}")
    await state.clear()
    await message.answer("Привет! Я - твой бот-помощник для обработки аудиозаписей лекций! \n Давайте зарегистрируем вашу лекцию.", reply_markup=ReplyKeyboardRemove())
    start_text = (
...
# --- Режим разработчика (обрабатывается в любом состоянии) ---
...
async def handle_correction_choice(message: Message, state: FSMContext):
    ...
    if choice == "1. Исправить название предмета":
        ...
        await state.set_state(LectureRegistration.correcting_discipline)
        ...
        await state.set_state(LectureRegistration.correcting_teacher)
        ...
        await state.set_state(LectureRegistration.correcting_datetime)
    elif choice == "Отмена":
        user_data = await state.get_data()
        confirmation_message = (
            "📋 Пожалуйста, подтвердите введенные данные:\n\n"
...
@registration_router.message(StateFilter(LectureRegistration.correcting_discipline), F.text)
async def rehandle_discipline(message: Message, state: FSMContext):
    discipline_name = message.text.strip()
...
@registration_router.message(StateFilter(LectureRegistration.correcting_teacher), F.text)
async def rehandle_teacher_name(message: Message, state: FSMContext):
    teacher_name = message.text.strip()
...
@registration_router.message(StateFilter(LectureRegistration.correcting_datetime), F.text)
async def rehandle_datetime(message: Message, state: FSMContext):
    datetime_input = message.text.strip()
    if not datetime_input:
        await message.answer("Дата/время не могут быть пустыми.", reply_markup=ReplyKeyboardRemove())
        return
...
async def register_aiogram_handlers(dp: Dispatcher, bot: Bot):
    # Сначала обработчики команд, которые должны работать в любом состоянии
